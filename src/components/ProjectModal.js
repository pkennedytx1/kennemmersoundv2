          ...
          <Modal className='project-modal-container' show={props.show} onHide={props.handleClose}>
            ...
            <Modal.Body>
                ...
                {props.links && props.links.length > 0 ? props.links.map((link, i) => {
                    return(
                            <Button className='project-modal-links' variant='dark' target='_blank' href={link.link}>{link.type === 'content' ? <i class="fab fa-youtube"></i> : <i class="fas fa-users"></i>} {link.name}</Button>
                            )
                        }) : null }
                </Modal.Footer>
                : null}
